...
<head>
    ...
    <title><?= e('HaushaltsPilot – Teil 15 Kalender') ?></title>
...
<body>
    <div class="app-shell">
        <aside class="sidebar">
            ...
            <div class="sidebar-footer">Teil 15 · Kalender mit eigenen Terminen und Familienterminen.</div>
        </aside>
        ...
        <div class="main-shell">
            ...
            <main class="content">
                <section class="module-page active" id="dashboardPage">
                    <div class="dashboard-hero">
                        <section class="hero-card">
                            ...
                            <p>Teil 15 ergänzt einen eigenen Kalender mit Monatsansicht im FullCalendar-Stil, persönlichen Terminen und Familienterminen mit vollständigem CRUD.</p>
                ...
                <section class="module-page" id="calendarPage">
                    <div class="note">Teil 15 ergänzt einen eigengebauten Kalender im FullCalendar-Stil. Termine werden serverseitig als private Termine oder Familientermine gespeichert und über die Haushaltszuordnung freigegeben.</div>
                        ...
                        <section class="panel">
                            <div class="calendar-toolbar">
                                ...
                                <div class="calendar-actions">
                                    <button class="small-btn" type="button" id="calendarPrevMonth">‹ Monat</button>
                                    <button class="small-btn" type="button" id="calendarTodayButton">Heute</button>
                                    <button class="small-btn" type="button" id="calendarNextMonth">Monat ›</button>
                                </div>
                            </div>
                ...
                <p class="footer-note">Teil 15 erweitert HaushaltsPilot um eine eigene Kalenderoberfläche mit persönlichen und gemeinsamen Familienterminen.</p>
            </main>
        </div>
    </div>
    ...
    <script>
        ...
        function calendarPayloadFromInputs(prefix) {
            const source = prefix === 'modal' ? {
                title:calendarModalTitleInput, scope:calendarModalScopeInput, start:calendarModalStartInput, end:calendarModalEndInput, location:calendarModalLocationInput, description:calendarModalDescriptionInput
            } : {
                title:calendarTitleInput, scope:calendarScopeInput, start:calendarStartInput, end:calendarEndInput, location:calendarLocationInput, description:calendarDescriptionInput
            };
            return {
        ...
        async function createCalendarEventFromForm() {
            ...
            try {
                ...
                appData = result.data;
                calendarTitleInput.value=''; calendarLocationInput.value=''; calendarDescriptionInput.value=''; calendarEndInput.value='';
        ...
        function renderDashboardShell() {
            ...
            const unreadFamilyChats = Number(appData.totalUnreadFamilyChats || 0);
            const unread = unreadMessages + unreadChats + unreadFamilyChats;
            const todayKey = dateKeyFromDate(new Date());
            const upcomingEvents = events.filter(function(event){ return (event.startsAt || '').slice(0,10) >= todayKey; }).length;
            const familyEvents = events.filter(function(event){ return event.scope === 'family'; }).length;
            const adminObjectCount = (appData.families || []).length + (appData.users || []).length + lists.length + (appData.adminTodos || todos).length + (appData.adminMessages || []).length + (appData.adminCalendarEvents || events).length;
            ...
            setBadge(navCalendarBadge, events.length);
            setBadge(navFamilyChatBadge, unreadFamilyChats);
            ...
            dashCalendarCount.textContent = String(events.length);
            dashCalendarDetail.textContent = upcomingEvents + ' anstehend · ' + familyEvents + ' Familie';
            dashMessageDetail.textContent = unreadMessages + ' Nachrichten · ' + unreadChats + ' Chats · ' + unreadFamilyChats + ' Familie';
            dashAdminCount.textContent = String(appData.isAdmin === true ? adminObjectCount : 0);
            dashAdminDetail.textContent = appData.isAdmin === true ? 'Haushalte, Nutzer, Listen, Todos, Nachrichten, Kalender' : 'nur für Admins sichtbar';
            ...
            dashMessageSummary.textContent = unreadMessages + ' ungelesene private Nachricht(en), ' + unreadChats + ' ungelesene 1:1-Chatnachricht(en) und ' + unreadFamilyChats + ' ungelesene Familienchat-Nachricht(en).';
            dashCalendarSummary.textContent = events.length + ' sichtbare Termine, davon ' + familyEvents + ' Familientermin(e) und ' + upcomingEvents + ' anstehend.';
        }
        ...
        function openCalendarModal(eventOrDate) {
            ...
            if (event) {
                ...
                calendarModalMeta.textContent = eventScopeLabel(event.scope) + ' · Besitzer: ' + getUserName(event.ownerId);
        ...
        function renderCalendar() {
            ...
            const events = (appData.calendarEvents || []).slice().sort((a,b) => (a.startsAt || '').localeCompare(b.startsAt || ''));
            ...
            for (let index=0; index<42; index++) {
                ...
                events.filter((event)=>eventDateKey(event) === key).slice(0,4).forEach(function(event){
                    const chip=document.createElement('button'); chip.type='button'; chip.className='calendar-event-chip' + (event.scope === 'family' ? ' family' : ''); chip.textContent=eventTimeLabel(event) + ' ' + event.title; chip.title=event.title + (event.location ? ' · ' + event.location : ''); chip.addEventListener('click', function(ev){ ev.stopPropagation(); openCalendarModal(event.id); }); cell.appendChild(chip);
                });
                const count = events.filter((event)=>eventDateKey(event) === key).length;
                if (count > 4) { const more=document.createElement('small'); more.textContent='+' + (count-4) + ' weitere'; more.style.color='var(--muted)'; cell.appendChild(more); }
                cell.addEventListener('click', function(){ openCalendarModal(date); });
                calendarGrid.appendChild(cell);
            }
            renderCalendarUpcoming(events);
        ...
        function renderCalendarUpcoming(events) {
            ...
            const todayKey = dateKeyFromDate(new Date());
            const upcoming = events.filter((event)=>eventDateKey(event) >= todayKey).slice(0,12);
            ...
            upcoming.forEach(function(event){
                ...
                const meta=document.createElement('small'); meta.textContent=eventScopeLabel(event.scope) + ' · ' + event.startsAt + (event.endsAt ? ' bis ' + event.endsAt : '');
        ...
        calendarPrevMonth.addEventListener('click', function(){ calendarCursor.setMonth(calendarCursor.getMonth()-1); renderCalendar(); });
        calendarNextMonth.addEventListener('click', function(){ calendarCursor.setMonth(calendarCursor.getMonth()+1); renderCalendar(); });
