...
function visible_message_threads_for_user(array $data, array $user): array
{
    $visible = [];
...
function visible_messages_for_user(array $data, array $user): array
{
    $visibleThreadIds = array_map(
        static fn(array $thread): string => $thread['id'],
        visible_message_threads_for_user($data, $user)
    );
...
function find_personal_thread_index(array $data, string $firstUserId, string $secondUserId): ?int
{
    $wanted = [$firstUserId, $secondUserId];
    sort($wanted);

    foreach ($data['messageThreads'] as $index => $thread) {
        if ($thread['threadType'] !== MESSAGE_THREAD_PERSONAL) {
            continue;
        }
...
function prepare_response_data(array $data, array $user): array
{
    ...
    $visibleThreads = visible_message_threads_for_user($data, $user);
    ...
    return [
        ...
        'adminMessages' => is_admin($user) ? $data['messages'] : [],
...
switch ($action) {
    ...
    case 'send_message':
        ...
        $now = now_string();
        $threadIndex = find_personal_thread_index($data, $currentUser['id'], $recipientId);

        if ($threadIndex === null) {
            $thread = [
                'id' => create_id('thread'),
                'threadType' => MESSAGE_THREAD_PERSONAL,
        ...
        save_and_send($storageAdapter, $data, $currentUser, 'Nachricht wurde gesendet.');
