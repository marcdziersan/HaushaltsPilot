...
<head>
    ...
    <title><?= e('HaushaltsPilot – Teil 12 Personal Messages') ?></title>
    ...
    <style>
        ...
        * { box-sizing:border-box; }
        ...
        @media(max-width:1100px){ .small-boxes{grid-template-columns:repeat(2,1fr);} .dashboard-hero,.dashboard-grid,.layout,.message-layout,.todo-board{grid-template-columns:1fr;} .todo-form,.todo-edit-form,.todo-dashboard{grid-template-columns:1fr;} }
...
<body>
    <div class="app-shell">
        <aside class="sidebar">
            ...
            <div class="sidebar-footer">Teil 12 · AdminLTE-inspirierte App-Shell mit Dashboard, Modulen, Badges und getrennten Arbeitsbereichen.</div>
        </aside>
        ...
        <div class="main-shell">
            ...
            <main class="content">
                <section class="module-page active" id="dashboardPage">
                    <div class="dashboard-hero">
                        <section class="hero-card">
                            ...
                            <p>Teil 12 führt die Anwendung als echtes Dashboard zusammen: Listen, vertiefte Todos, private Nachrichten und Administration sind nicht mehr untereinander gestapelt, sondern als Module in einer App-Oberfläche organisiert.</p>
                    ...
                    <section class="small-boxes" aria-label="Kennzahlen">
                        ...
                        <div class="small-box box-yellow"><strong id="dashUnreadCount">0</strong><span>Ungelesene Nachrichten</span><small id="dashMessageDetail">Private Messages</small><span class="big-icon">✉</span></div>
                    ...
                    <section class="dashboard-grid">
                        <div class="panel">
                            ...
                            <div class="dashboard-card-grid">
                                ...
                                <article class="dashboard-card"><strong>Nächster Ausbau</strong><span>Teil 13 kann auf dem Nachrichtenmodell mit einem echten 1:1-Chat aufbauen.</span></article>
                            </div>
                        </div>
                        <div class="panel">
                ...
                <section class="module-page panel message-panel" id="messagesPage">
                    ...
                    <p>Teil 12 ergänzt private Nachrichten zwischen Nutzern. Ungelesene Nachrichten werden als Badge mit Zähler angezeigt und pro Verlauf getrennt berechnet.</p>
                ...
                <p class="footer-note">Teil 13 kann darauf mit einem echten 1:1-Chat aufbauen.</p>
            </main>
        </div>
    </div>
    ...
    <script>
        ...
        const modulePages = { dashboard:byId('dashboardPage'), lists:byId('listsPage'), todos:byId('todosPage'), messages:byId('messagesPage'), admin:byId('adminPanel') };
        ...
        let appData = { currentUser:null, currentFamily:null, lists:[], activeListId:null, todos:[], adminTodos:[], messageThreads:[], messages:[], totalUnreadMessages:0, adminMessages:[], users:[], activeUsers:[], families:[], isAdmin:false };
        ...
        function getThread(threadId) { return (appData.messageThreads || []).find((thread) => thread.id === threadId) || null; }
        ...
        function findThreadByOtherUser(userId) { return (appData.messageThreads || []).find((thread) => thread.participantIds.includes(userId)); }
        async function sendPersonalMessage(recipientId, body) {
            ...
            try {
                const result = await apiPost('send_message', { recipientId, body:body.trim() });
                appData = result.data;
                const thread = findThreadByOtherUser(recipientId);
        ...
        function renderApp() { renderCurrentUser(); renderUserSelects(); renderListGroups(); renderActiveList(); renderTodos(); renderMessages(); renderAdminPanel(); renderDashboardShell(); }
        ...
        function renderDashboardShell() {
            ...
            const unread = Number(appData.totalUnreadMessages || 0);
            ...
            setBadge(navUnreadBadge, unread);
            ...
            dashMessageDetail.textContent = (appData.messageThreads || []).length + ' Verlauf/Verläufe';
            ...
            dashMessageSummary.textContent = unread + ' ungelesene Nachricht(en) in ' + (appData.messageThreads || []).length + ' Verlauf/Verläufen.';
        }
        ...
        function renderMessages() {
            ...
            if (activeThreadId && !getThread(activeThreadId)) activeThreadId = null;
            threads.forEach(function(thread){ renderThreadButton(thread); });
            renderActiveThread();
        }
        function renderThreadButton(thread) {
            const button=document.createElement('button'); button.className='thread-button'; button.type='button'; if (thread.id===activeThreadId) button.classList.add('active');
            ...
            button.addEventListener('click', function(){ openThread(thread.id); }); threadList.appendChild(button);
        }
        ...
        messageForm.addEventListener('submit', function(event){ event.preventDefault(); const body=messageBodyInput.value; sendPersonalMessage(messageRecipientInput.value, body).then(function(){ messageBodyInput.value=''; }); });
        replyForm.addEventListener('submit', function(event){ event.preventDefault(); const thread=getThread(activeThreadId); const other=getOtherParticipant(thread); if (!thread || !other) { showStatus('Kein Nachrichtenverlauf ausgewählt.', true); return; } const body=replyInput.value; sendPersonalMessage(other.id, body).then(function(){ replyInput.value=''; }); });
