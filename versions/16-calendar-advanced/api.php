...
function visible_calendar_events_for_user(array $data, array $user): array
{
    $visible = [];

    foreach ($data['calendarEvents'] as $event) {
        if (user_can_view_calendar_event($user, $event)) {
            $visible[] = $event;
        }
    }

    usort($visible, static fn(array $left, array $right): int => strcmp($left['startsAt'], $right['startsAt']));
    return $visible;
}

function public_calendar_event(array $event): array
{
    return [
        'id' => $event['id'],
        ...
        'recurrence' => $event['recurrence'] ?? CALENDAR_RECURRENCE_NONE,
...
function clean_calendar_datetime(mixed $value, string $fieldLabel): string
{
    if (!is_string($value)) {
        send_json(['success' => false, 'message' => $fieldLabel . ' ist ungültig.'], 400);
    }

    $value = trim($value);
    require_non_empty($value, $fieldLabel . ' muss angegeben werden.');
    ...
    send_json(['success' => false, 'message' => $fieldLabel . ' muss im Format JJJJ-MM-TT HH:MM angegeben werden.'], 400);
}

function clean_optional_calendar_datetime(mixed $value, string $fieldLabel): string
{
    if ($value === null || $value === '') {
        return '';
    }

    return clean_calendar_datetime($value, $fieldLabel);
}


function clean_calendar_recurrence(mixed $value): string
{
    if (!is_string($value)) {
    ...
    if (!in_array($value, ALLOWED_CALENDAR_RECURRENCES, true)) {
        send_json(['success' => false, 'message' => 'Ungültige Wiederholung.'], 400);
    }

    return $value;
}

function clean_optional_calendar_date(mixed $value, string $fieldLabel): string
{
    if ($value === null || $value === '') {
        return '';
    }
    ...
    if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $value)) {
        send_json(['success' => false, 'message' => $fieldLabel . ' muss im Format JJJJ-MM-TT angegeben werden.'], 400);
    }

    return $value;
}

function clean_calendar_todo_link(array $data, array $actor, mixed $value): string
{
    if ($value === null || $value === '') {
        return '';
    }

    $todoId = clean_text($value, MAX_CALENDAR_TODO_ID_LENGTH);
    ...
    if ($todoIndex === null || !user_can_view_todo($actor, $data['todos'][$todoIndex])) {
        send_json(['success' => false, 'message' => 'Die verknüpfte Aufgabe wurde nicht gefunden oder ist nicht freigegeben.'], 403);
    }

    return $todoId;
}

function normalize_calendar_event_for_scope(array $actor, string $scope, string $title, string $startsAt, string $endsAt, string $location, string $description, string $recurrence, string $recurrenceUntil, string $reminderAt, string $todoId): array
{
    if ($scope === TODO_SCOPE_FAMILY && $actor['familyId'] === '') {
        send_json(['success' => false, 'message' => 'Familientermine benötigen einen Haushalt. Weise dich zuerst einem Haushalt zu.'], 400);
    }

    if ($endsAt !== '' && $endsAt < $startsAt) {
        send_json(['success' => false, 'message' => 'Das Ende darf nicht vor dem Beginn liegen.'], 400);
    }

    if ($recurrence !== CALENDAR_RECURRENCE_NONE && $recurrenceUntil === '') {
        send_json(['success' => false, 'message' => 'Wiederholungstermine benötigen ein Enddatum.'], 400);
    }

    if ($recurrenceUntil !== '' && $recurrenceUntil < substr($startsAt, 0, 10)) {
        send_json(['success' => false, 'message' => 'Die Wiederholung darf nicht vor dem Starttermin enden.'], 400);
    }

    if ($reminderAt !== '' && $reminderAt > $startsAt) {
        send_json(['success' => false, 'message' => 'Die Erinnerung darf nicht nach dem Beginn liegen.'], 400);
    }
    ...
    return [
        'id' => create_id('event'),
        'ownerId' => $actor['id'],
        'familyId' => $scope === TODO_SCOPE_FAMILY ? $actor['familyId'] : '',
        'scope' => $scope,
        'title' => $title,
        'startsAt' => $startsAt,
        'endsAt' => $endsAt,
...
function prepare_response_data(array $data, array $user): array
{
    ...
    $visibleCalendarEvents = visible_calendar_events_for_user($data, $user);
    ...
    return [
        ...
        'calendarEvents' => array_map('public_calendar_event', $visibleCalendarEvents),
        'adminCalendarEvents' => is_admin($user) ? array_map('public_calendar_event', $data['calendarEvents']) : [],
...
function update_owned_calendar_events_family(array &$data, string $ownerId, string $familyId): void
{
    foreach ($data['calendarEvents'] as &$event) {
        if ($event['ownerId'] === $ownerId) {
            if ($event['scope'] === TODO_SCOPE_FAMILY) {
                $event['familyId'] = $familyId;
                if ($familyId === '') {
                    $event['scope'] = TODO_SCOPE_PRIVATE;
                }
                $event['updatedAt'] = now_string();
            }
        }
    }
    unset($event);
...
switch ($action) {
    ...
    case 'create_calendar_event':
        $title = clean_text($input['title'] ?? '', MAX_CALENDAR_TITLE_LENGTH);
        $scope = clean_todo_scope($input['scope'] ?? TODO_SCOPE_PRIVATE);
        $startsAt = clean_calendar_datetime($input['startsAt'] ?? '', 'Beginn');
        $endsAt = clean_optional_calendar_datetime($input['endsAt'] ?? '', 'Ende');
        $location = clean_text($input['location'] ?? '', MAX_CALENDAR_LOCATION_LENGTH);
        $description = clean_text($input['description'] ?? '', MAX_CALENDAR_DESCRIPTION_LENGTH);
        $recurrence = clean_calendar_recurrence($input['recurrence'] ?? CALENDAR_RECURRENCE_NONE);
        $recurrenceUntil = clean_optional_calendar_date($input['recurrenceUntil'] ?? '', 'Wiederholen bis');
        $reminderAt = clean_optional_calendar_datetime($input['reminderAt'] ?? '', 'Erinnerung');
        $todoId = clean_calendar_todo_link($data, $currentUser, $input['todoId'] ?? '');
        require_non_empty($title, 'Bitte gib einen Termintitel ein.');

        $data['calendarEvents'][] = normalize_calendar_event_for_scope($currentUser, $scope, $title, $startsAt, $endsAt, $location, $description, $recurrence, $recurrenceUntil, $reminderAt, $todoId);

        save_and_send($storageAdapter, $data, $currentUser, 'Termin wurde erstellt.');
    ...
    case 'update_calendar_event':
        ...
        $reminderAt = clean_optional_calendar_datetime($input['reminderAt'] ?? '', 'Erinnerung');
        $todoId = clean_calendar_todo_link($data, $currentUser, $input['todoId'] ?? '');
        require_non_empty($eventId, 'Kein Termin angegeben.');
        require_non_empty($title, 'Bitte gib einen Termintitel ein.');

        $eventIndex = find_calendar_event_index($data, $eventId);

        if ($eventIndex === null || !user_can_manage_calendar_event($currentUser, $data['calendarEvents'][$eventIndex])) {
            send_json(['success' => false, 'message' => 'Du darfst diesen Termin nicht bearbeiten.'], 403);
        }

        $targetFamilyId = '';

        if ($scope === TODO_SCOPE_FAMILY) {
            $targetFamilyId = $currentUser['familyId'] !== ''
                ? $currentUser['familyId']
                : ($data['calendarEvents'][$eventIndex]['familyId'] ?? '');

            if ($targetFamilyId === '') {
                send_json(['success' => false, 'message' => 'Familientermine benötigen einen Haushalt.'], 400);
            }
        }
        ...
        $data['calendarEvents'][$eventIndex]['title'] = $title;
        $data['calendarEvents'][$eventIndex]['scope'] = $scope;
        $data['calendarEvents'][$eventIndex]['familyId'] = $scope === TODO_SCOPE_FAMILY ? $targetFamilyId : '';
        $data['calendarEvents'][$eventIndex]['startsAt'] = $startsAt;
        $data['calendarEvents'][$eventIndex]['endsAt'] = $endsAt;
        $data['calendarEvents'][$eventIndex]['location'] = $location;
        $data['calendarEvents'][$eventIndex]['description'] = $description;
        $data['calendarEvents'][$eventIndex]['recurrence'] = $recurrence;
        $data['calendarEvents'][$eventIndex]['recurrenceUntil'] = $recurrenceUntil;
        $data['calendarEvents'][$eventIndex]['reminderAt'] = $reminderAt;
        $data['calendarEvents'][$eventIndex]['todoId'] = $todoId;
        $data['calendarEvents'][$eventIndex]['updatedAt'] = now_string();

        save_and_send($storageAdapter, $data, $currentUser, 'Termin wurde aktualisiert.');
    ...
    case 'delete_calendar_event':
        ...
        if ($eventIndex === null || !user_can_manage_calendar_event($currentUser, $data['calendarEvents'][$eventIndex])) {
            send_json(['success' => false, 'message' => 'Du darfst diesen Termin nicht löschen.'], 403);
        }

        array_splice($data['calendarEvents'], $eventIndex, 1);

        save_and_send($storageAdapter, $data, $currentUser, 'Termin wurde gelöscht.');
    ...
    case 'admin_update_user_family':
        ...
        update_owned_calendar_events_family($data, $userId, $familyId);
    ...
    case 'admin_delete_user':
        ...
        $data['calendarEvents'] = array_values(array_filter($data['calendarEvents'], function (array $event) use ($userId): bool {
