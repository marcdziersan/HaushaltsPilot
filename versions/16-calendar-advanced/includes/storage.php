...
final class PdoStorage implements AppStorageInterface
{
    ...
    public function load(): array
    {
        ...
        $calendarRows = $this->pdo
            ->query('SELECT id, owner_id, family_id, scope, title, starts_at, ends_at, location, description, recurrence, recurrence_until, reminder_at, todo_id, created_at, updated_at FROM calendar_events ORDER BY starts_at ASC, created_at DESC')
            ->fetchAll();
        ...
        foreach ($calendarRows as $calendarRow) {
            $calendarEvents[] = [
                ...
                'ownerId' => (string) $calendarRow['owner_id'],
                'familyId' => (string) ($calendarRow['family_id'] ?? ''),
                'scope' => (string) $calendarRow['scope'],
                'title' => (string) $calendarRow['title'],
                'startsAt' => (string) $calendarRow['starts_at'],
                'endsAt' => (string) ($calendarRow['ends_at'] ?? ''),
                'location' => (string) ($calendarRow['location'] ?? ''),
                'description' => (string) ($calendarRow['description'] ?? ''),
                'recurrence' => (string) ($calendarRow['recurrence'] ?? CALENDAR_RECURRENCE_NONE),
                'recurrenceUntil' => (string) ($calendarRow['recurrence_until'] ?? ''),
                'reminderAt' => (string) ($calendarRow['reminder_at'] ?? ''),
                'todoId' => (string) ($calendarRow['todo_id'] ?? ''),
                'createdAt' => (string) $calendarRow['created_at'],
                'updatedAt' => (string) ($calendarRow['updated_at'] ?? $calendarRow['created_at']),
            ];
        }
    ...
    public function save(array $data): void
    {
        ...
        try {
            ...
            $calendarInsert = $this->pdo->prepare(
                'INSERT INTO calendar_events (id, owner_id, family_id, scope, title, starts_at, ends_at, location, description, recurrence, recurrence_until, reminder_at, todo_id, created_at, updated_at)
                 VALUES (:id, :owner_id, :family_id, :scope, :title, :starts_at, :ends_at, :location, :description, :recurrence, :recurrence_until, :reminder_at, :todo_id, :created_at, :updated_at)'
            );
            ...
            foreach ($data['calendarEvents'] as $event) {
                $calendarInsert->execute([
                    ...
                    ':ends_at' => $event['endsAt'] !== '' ? $event['endsAt'] : null,
                    ':location' => $event['location'],
                    ':description' => $event['description'],
                    ':recurrence' => $event['recurrence'] ?? CALENDAR_RECURRENCE_NONE,
                    ':recurrence_until' => ($event['recurrenceUntil'] ?? '') !== '' ? $event['recurrenceUntil'] : null,
                    ':reminder_at' => ($event['reminderAt'] ?? '') !== '' ? $event['reminderAt'] : null,
                    ':todo_id' => ($event['todoId'] ?? '') !== '' ? $event['todoId'] : null,
...
SQL);
        ...
        try_add_schema_column($pdo, "ALTER TABLE calendar_events ADD COLUMN location TEXT NOT NULL DEFAULT ''");
        try_add_schema_column($pdo, "ALTER TABLE calendar_events ADD COLUMN description TEXT NOT NULL DEFAULT ''");
        try_add_schema_column($pdo, "ALTER TABLE calendar_events ADD COLUMN recurrence TEXT NOT NULL DEFAULT 'none'");
        try_add_schema_column($pdo, "ALTER TABLE calendar_events ADD COLUMN recurrence_until TEXT NULL");
        try_add_schema_column($pdo, "ALTER TABLE calendar_events ADD COLUMN reminder_at TEXT NULL");
        try_add_schema_column($pdo, "ALTER TABLE calendar_events ADD COLUMN todo_id TEXT NULL");
        try_add_schema_column($pdo, "ALTER TABLE calendar_events ADD COLUMN updated_at TEXT NOT NULL DEFAULT ''");
...
SQL);
        ...
        $pdo->exec(
            'CREATE TABLE IF NOT EXISTS calendar_events (
                ...
                INDEX idx_calendar_events_reminder_at (reminder_at),
                INDEX idx_calendar_events_todo_id (todo_id),
                CONSTRAINT fk_calendar_events_owner FOREIGN KEY (owner_id) REFERENCES users(id) ON DELETE CASCADE
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci'
        );
        try_add_schema_column($pdo, "ALTER TABLE calendar_events ADD COLUMN location VARCHAR(180) NOT NULL DEFAULT ''");
        try_add_schema_column($pdo, "ALTER TABLE calendar_events ADD COLUMN description TEXT NULL");
        try_add_schema_column($pdo, "ALTER TABLE calendar_events ADD COLUMN recurrence ENUM('none', 'daily', 'weekly', 'monthly') NOT NULL DEFAULT 'none'");
        try_add_schema_column($pdo, "ALTER TABLE calendar_events ADD COLUMN recurrence_until DATE NULL");
        try_add_schema_column($pdo, "ALTER TABLE calendar_events ADD COLUMN reminder_at DATETIME NULL");
        try_add_schema_column($pdo, "ALTER TABLE calendar_events ADD COLUMN todo_id VARCHAR(80) NULL");
        try_add_schema_column($pdo, "ALTER TABLE calendar_events ADD COLUMN updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP");
    }
}
    ...
    return [
        ...
        'todos' => [
            [
                ...
                'title' => 'Teil 16 testen: Kalender vertiefen',
        ...
        'calendarEvents' => [
            [
                ...
                'familyId' => $familyId,
                'scope' => TODO_SCOPE_PRIVATE,
                'title' => 'Persönlichen Termin testen',
                'startsAt' => date('Y-m-d 10:00:00', strtotime('+1 day')),
                'endsAt' => date('Y-m-d 11:00:00', strtotime('+1 day')),
                'location' => 'Zuhause',
                'description' => 'Demo-Termin für Teil 16 mit Erinnerungs- und Wiederholungsfeldern.',
                'recurrence' => CALENDAR_RECURRENCE_NONE,
                'recurrenceUntil' => '',
                'reminderAt' => date('Y-m-d 09:30:00', strtotime('+1 day')),
                'todoId' => '',
                'createdAt' => $now,
                'updatedAt' => $now,
            ],
            [
                ...
                'familyId' => $familyId,
                'scope' => TODO_SCOPE_FAMILY,
                'title' => 'Familientermin planen',
                'startsAt' => date('Y-m-d 16:00:00', strtotime('+2 days')),
                'endsAt' => date('Y-m-d 17:00:00', strtotime('+2 days')),
                'location' => 'Haushalt',
                'description' => 'Gemeinsamer Beispieltermin für alle Haushaltsmitglieder.',
                'recurrence' => CALENDAR_RECURRENCE_WEEKLY,
                'recurrenceUntil' => date('Y-m-d', strtotime('+6 weeks')),
                'reminderAt' => date('Y-m-d 15:30:00', strtotime('+2 days')),
                'todoId' => '',
                'createdAt' => $now,
                'updatedAt' => $now,
            ],
