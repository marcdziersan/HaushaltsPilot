...
const MAX_CALENDAR_LOCATION_LENGTH = 120;
const MAX_CALENDAR_DESCRIPTION_LENGTH = 1200;
const MAX_CALENDAR_DATETIME_LENGTH = 16;
const MAX_CALENDAR_RECURRENCE_LENGTH = 20;
const MAX_CALENDAR_TODO_ID_LENGTH = 80;
const CALENDAR_RECURRENCE_NONE = 'none';
const CALENDAR_RECURRENCE_DAILY = 'daily';
const CALENDAR_RECURRENCE_WEEKLY = 'weekly';
const CALENDAR_RECURRENCE_MONTHLY = 'monthly';
const ALLOWED_CALENDAR_RECURRENCES = [CALENDAR_RECURRENCE_NONE, CALENDAR_RECURRENCE_DAILY, CALENDAR_RECURRENCE_WEEKLY, CALENDAR_RECURRENCE_MONTHLY];
...
const MESSAGE_THREAD_LEGACY_DIRECT = 'direct';
const MESSAGE_THREAD_PERSONAL = 'personal_message';
const MESSAGE_THREAD_CHAT = 'one_to_one_chat';
const MESSAGE_THREAD_FAMILY = 'family_chat';
...
const ALLOWED_MESSAGE_THREAD_TYPES = [
    MESSAGE_THREAD_LEGACY_DIRECT,
    MESSAGE_THREAD_PERSONAL,
    MESSAGE_THREAD_CHAT,
    MESSAGE_THREAD_FAMILY,
];
...
function normalize_app_data(array $data): array
{
    ...
    foreach ($data['calendarEvents'] as &$event) {
        if (!isset($event['familyId']) || !is_string($event['familyId'])) {
            $event['familyId'] = '';
        }
        if (!isset($event['scope']) || !in_array($event['scope'], ALLOWED_TODO_SCOPES, true)) {
        ...
        if (!isset($event['endsAt']) || !is_string($event['endsAt'])) {
            $event['endsAt'] = '';
        }
        if (!isset($event['updatedAt']) || !is_string($event['updatedAt'])) {
            $event['updatedAt'] = $event['createdAt'] ?? now_string();
        }
        if (!isset($event['recurrence']) || !is_string($event['recurrence']) || !in_array($event['recurrence'], ALLOWED_CALENDAR_RECURRENCES, true)) {
            $event['recurrence'] = CALENDAR_RECURRENCE_NONE;
        }
        if (!isset($event['recurrenceUntil']) || !is_string($event['recurrenceUntil'])) {
            $event['recurrenceUntil'] = '';
        }
        if (!isset($event['reminderAt']) || !is_string($event['reminderAt'])) {
            $event['reminderAt'] = '';
        }
        if (!isset($event['todoId']) || !is_string($event['todoId'])) {
            $event['todoId'] = '';
        }
    }
    unset($event);
    ...
    foreach ($data['typingIndicators'] as &$indicator) {
        ...
        if (!isset($indicator['channel']) || !in_array($indicator['channel'], ['message', 'chat', 'family'], true)) {
    ...
    foreach ($data['messageThreads'] as &$thread) {
        ...
        if (!isset($thread['familyId']) || !is_string($thread['familyId'])) {
            $thread['familyId'] = '';
        }
        if (!isset($thread['title']) || !is_string($thread['title'])) {
            $thread['title'] = '';
        }
...
function is_valid_data(mixed $data): bool
{
    if (
        !is_array($data) ||
        !isset($data['families'], $data['users'], $data['lists'], $data['activeLists'], $data['todos'], $data['messageThreads'], $data['messages'], $data['typingIndicators'], $data['calendarEvents']) ||
    ...
    foreach ($data['calendarEvents'] as $event) {
        if (
            !is_valid_calendar_event($event) ||
            !in_array($event['ownerId'], $userIds, true) ||
            !in_array($event['familyId'], $familyIds, true)
        ) {
            return false;
        }
    }
    ...
    foreach ($data['messageThreads'] as $thread) {
        ...
        if ($thread['threadType'] === MESSAGE_THREAD_FAMILY && ($thread['familyId'] === '' || !in_array($thread['familyId'], $familyIds, true))) {
            return false;
        }
...
function is_valid_calendar_event(mixed $event): bool
{
    return (
        is_array($event) &&
        isset($event['id'], $event['ownerId'], $event['familyId'], $event['scope'], $event['title'], $event['startsAt'], $event['endsAt'], $event['location'], $event['description'], $event['recurrence'], $event['recurrenceUntil'], $event['reminderAt'], $event['todoId'], $event['createdAt'], $event['updatedAt']) &&
        is_string($event['id']) &&
        is_string($event['ownerId']) &&
        is_string($event['familyId']) &&
        in_array($event['scope'], ALLOWED_TODO_SCOPES, true) &&
        is_string($event['title']) &&
        is_string($event['startsAt']) &&
        is_string($event['endsAt']) &&
        is_string($event['location']) &&
        is_string($event['description']) &&
        in_array($event['recurrence'], ALLOWED_CALENDAR_RECURRENCES, true) &&
        is_string($event['recurrenceUntil']) &&
        is_string($event['reminderAt']) &&
        is_string($event['todoId']) &&
        is_string($event['createdAt']) &&
        is_string($event['updatedAt'])
    );
}
...
function is_valid_message_thread(mixed $thread): bool
{
    return (
        is_array($thread) &&
        isset($thread['id'], $thread['threadType'], $thread['participantIds'], $thread['familyId'], $thread['title'], $thread['lastReadAt'], $thread['createdBy'], $thread['createdAt'], $thread['updatedAt']) &&
        is_string($thread['id']) &&
        in_array($thread['threadType'], ALLOWED_MESSAGE_THREAD_TYPES, true) &&
        is_string($thread['familyId']) &&
        is_string($thread['title']) &&
...
function is_valid_typing_indicator(mixed $indicator): bool
{
    return (
        ...
        in_array($indicator['channel'], ['message', 'chat', 'family'], true) &&
...
function find_calendar_event_index(array $data, string $eventId): ?int
{
    foreach ($data['calendarEvents'] as $index => $event) {
        if ($event['id'] === $eventId) {
            return $index;
        }
    }
...
function user_can_view_message_thread(array $user, array $thread): bool
{
    ...
    if (($thread['threadType'] ?? '') === MESSAGE_THREAD_FAMILY) {
        return $user['familyId'] !== '' && ($thread['familyId'] ?? '') === $user['familyId'];
    }
...
function user_can_view_calendar_event(array $user, array $event): bool
{
    return is_admin($user) ||
        ($event['scope'] === TODO_SCOPE_PRIVATE && $event['ownerId'] === $user['id']) ||
        ($event['scope'] === TODO_SCOPE_FAMILY && $user['familyId'] !== '' && $event['familyId'] === $user['familyId']);
}

function user_can_manage_calendar_event(array $user, array $event): bool
{
    return user_can_view_calendar_event($user, $event);
}
