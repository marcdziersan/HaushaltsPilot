...
<head>
    ...
    <title><?= e('HaushaltsPilot – Teil 08 Persönliche und gemeinsame Listen') ?></title>
    ...
    <style>
        ...
        * { box-sizing:border-box; }
        ...
        .app { width:100%; max-width:1220px; margin:0 auto; background:var(--card); border-radius:var(--radius); box-shadow:var(--shadow); padding:24px; }
        ...
        .list-form, .item-form, .settings-form { display:grid; gap:10px; margin-bottom:12px; }
        ...
        .add-btn { background:var(--primary); color:white; font-weight:bold; }
        .add-btn:hover { background:var(--primary-dark); }
        .delete-list-btn { width:100%; background:var(--danger); color:white; font-weight:bold; margin-top:12px; }
        .delete-list-btn:hover { background:var(--danger-dark); }
        ...
        .small-success { background:var(--success); }
        .small-warning { background:var(--warning); color:white; }
        .scope-row { display:flex; gap:8px; align-items:center; color:var(--muted); font-size:.88rem; }
        .scope-row input { width:auto; }
        .admin-create-owner { display:none; }
        .admin-create-owner.visible { display:block; }
        ...
        .toggle-btn:hover { background:var(--success-dark); }
        ...
        .data-table { width:100%; border-collapse:collapse; background:white; border-radius:14px; overflow:hidden; }
        .data-table th, .data-table td { border-bottom:1px solid var(--border); padding:10px; text-align:left; font-size:.9rem; vertical-align:top; }
        .row-actions { display:flex; flex-wrap:wrap; gap:6px; align-items:center; }
        .row-actions select { width:auto; min-width:150px; padding:7px 9px; border-radius:9px; font-size:.82rem; }
        .footer-note { margin-top:22px; font-size:.82rem; color:var(--muted); text-align:center; line-height:1.4; }
        @media(max-width:940px){ .layout{grid-template-columns:1fr;} .item-form{grid-template-columns:1fr;} .topbar{display:block}.userbox{text-align:left;margin-top:14px} }
        @media(max-width:560px){ body{padding:12px}.app{padding:18px} h1{font-size:1.5rem}.stats{grid-template-columns:1fr}.shopping-item{grid-template-columns:1fr}.item-actions{width:100%}.toggle-btn,.delete-btn{flex:1}.current-title{display:block}.current-title span{display:block;margin-top:4px}.data-table{display:block;overflow-x:auto}.owner-select-row{grid-template-columns:1fr;} }
    </style>
</head>
<body>
    <main class="app">
        <header class="topbar">
            <div>
                ...
                <p>Teil 08: Persönliche Listen, Gemeinschaftslisten, Besitzer und Sichtbarkeit.</p>
            </div>
            <div class="userbox">
        ...
        <div class="security-note">
            Rechtebasis: Nutzer verwalten eigene Listen und Inhalte in Gemeinschaftslisten. Sichtbarkeit, Besitzerwechsel und Löschen sind auf Besitzer oder Admin beschränkt. Admins können alle Listen sehen, filtern und zuweisen. Diese Struktur bereitet Todos, Personal Messages, Einzelchat, Familienchat und Kalender vor.
        </div>
        ...
        <section class="layout">
            <aside class="panel">
                ...
                <form class="list-form" id="listForm">
                    ...
                    <div class="admin-create-owner" id="adminCreateOwnerBox">
                        <select id="listOwnerInput" aria-label="Besitzer für neue Liste"></select>
                    </div>
                    <button class="add-btn" type="submit">Liste erstellen</button>
                </form>

                <div class="list-group">
                    <div class="list-group-title"><span>Meine Listen</span><small id="myListsCount">0</small></div>
                    <div class="list-buttons" id="myListButtons"></div>
                </div>

                <div class="list-group">
                    <div class="list-group-title"><span>Gemeinschaftslisten</span><small id="sharedListsCount">0</small></div>
                    <div class="list-buttons" id="sharedListButtons"></div>
                </div>

                <div class="list-group" id="adminOtherListsGroup">
                    <div class="list-group-title"><span>Andere Nutzerlisten</span><small id="otherListsCount">0</small></div>
                    <div class="list-buttons" id="otherListButtons"></div>
                </div>
            ...
            <section class="panel">
                ...
                <div class="list-settings" id="listSettingsBox">
                    <div class="settings-header">
                        <div>
                            <strong>Listeneinstellungen</strong>
                            <span id="listSettingsInfo">Nur Besitzer oder Admins dürfen diese Werte ändern.</span>
                        </div>
                    </div>
                    <div class="settings-actions">
                        <button class="small-btn small-success" type="button" id="toggleSharedButton">Sichtbarkeit ändern</button>
                    </div>
                    <div class="owner-select-row" id="ownerSelectRow">
                        <select id="activeListOwnerSelect" aria-label="Listenbesitzer ändern"></select>
                        <button class="small-btn" type="button" id="updateOwnerButton">Besitzer speichern</button>
                    </div>
                </div>
        ...
        <section class="panel admin-panel" id="adminPanel">
            ...
            <div class="admin-grid">
                <section>
                    <h3>Listenverwaltung</h3>
                    <div class="table-toolbar">
                        <select id="adminListFilter" aria-label="Listen filtern">
                            <option value="all">Alle Listen</option>
                            <option value="mine">Meine Listen</option>
                            <option value="shared">Gemeinschaftslisten</option>
                            <option value="private">Private Listen</option>
                        </select>
                    </div>
                    <table class="data-table">
                        <thead><tr><th>Liste</th><th>Besitzer</th><th>Sichtbarkeit</th><th>Artikel</th><th>Aktionen</th></tr></thead>
        ...
        <p class="footer-note">Teil 09 kann darauf Familien- oder Haushaltsgruppen aufbauen. Die Rechteidee bleibt gleich: privat, gemeinsam, Besitzer, Admin.</p>
    </main>
    ...
    <script>
        ...
        const otherListsCount = document.getElementById("otherListsCount");
        ...
        const listSettingsBox = document.getElementById("listSettingsBox");
        const listSettingsInfo = document.getElementById("listSettingsInfo");
        const toggleSharedButton = document.getElementById("toggleSharedButton");
        ...
        let appData = { currentUser: null, lists: [], activeListId: null, users: [], activeUsers: [], isAdmin: false };
        ...
        function canManageListSettings(list) {
            return appData.isAdmin === true || (appData.currentUser && list.ownerId === appData.currentUser.id);
        }
        ...
        async function createList() {
            ...
            if (appData.isAdmin === true && listOwnerInput.value !== "") {
                payload.ownerId = listOwnerInput.value;
            }
            try {
                const result = await apiPost("create_list", payload);
                appData = result.data;
                listNameInput.value = "";
        ...
        async function deleteActiveList() {
            ...
            if (!canManageListSettings(activeList)) {
                showStatus("Nur Besitzer oder Admins dürfen Listen löschen.", true);
                return;
            }
        ...
        async function updateActiveListVisibility() {
            const activeList = getActiveList();
            if (!activeList) return;
            try {
                const result = await apiPost("update_list_visibility", {
                    listId: activeList.id,
                    isShared: !activeList.isShared
                });
                appData = result.data;
                renderApp();
                showStatus(result.message);
        ...
        async function updateActiveListOwner() {
            const activeList = getActiveList();
            if (!activeList || appData.isAdmin !== true) return;
            try {
                const result = await apiPost("admin_update_list_owner", {
                    listId: activeList.id,
                    ownerId: activeListOwnerSelect.value
                });
                appData = result.data;
                renderApp();
                showStatus(result.message);
        ...
        async function adminUpdateListOwner(listId, ownerId) {
            try {
                const result = await apiPost("admin_update_list_owner", { listId: listId, ownerId: ownerId });
                appData = result.data;
                renderApp();
                showStatus(result.message);
            } catch (error) { showStatus(error.message, true); }
        ...
        async function adminToggleListVisibility(list) {
            try {
                const result = await apiPost("update_list_visibility", { listId: list.id, isShared: !list.isShared });
                appData = result.data;
                renderApp();
                showStatus(result.message);
            } catch (error) { showStatus(error.message, true); }
        ...
        function renderUserSelects() {
            ...
            appData.activeUsers.forEach(function(user) {
                ...
                if (appData.currentUser && user.id === appData.currentUser.id) option.selected = true;
                listOwnerInput.appendChild(option);

                const activeOption = document.createElement("option");
                activeOption.value = user.id;
                activeOption.textContent = user.displayName + " (@" + user.username + ")";
                activeListOwnerSelect.appendChild(activeOption);
            });
        }

        function renderListGroups() {
            myListButtons.textContent = "";
            sharedListButtons.textContent = "";
            otherListButtons.textContent = "";

            const myLists = appData.lists.filter(function(list) { return appData.currentUser && list.ownerId === appData.currentUser.id; });
            const sharedLists = appData.lists.filter(function(list) { return appData.currentUser && list.isShared && list.ownerId !== appData.currentUser.id; });
            const otherLists = appData.lists.filter(function(list) { return appData.isAdmin && appData.currentUser && !list.isShared && list.ownerId !== appData.currentUser.id; });

            renderListButtonSet(myListButtons, myLists, "Keine eigenen Listen vorhanden.");
            renderListButtonSet(sharedListButtons, sharedLists, "Keine Gemeinschaftslisten sichtbar.");
            renderListButtonSet(otherListButtons, otherLists, "Keine fremden privaten Listen sichtbar.");

            myListsCount.textContent = String(myLists.length);
            sharedListsCount.textContent = String(sharedLists.length);
            otherListsCount.textContent = String(otherLists.length);
            adminOtherListsGroup.style.display = appData.isAdmin ? "block" : "none";

            const activeList = getActiveList();
            deleteListButton.disabled = !activeList || !canManageListSettings(activeList);
        }

        function renderListButtonSet(container, lists, emptyText) {
            if (lists.length === 0) {
        ...
        function renderActiveList() {
            ...
            if (!activeList) {
                ...
                listSettingsBox.classList.remove("visible");
                updateStats([]);
                return;
            }
        ...
        function renderListSettings(activeList) {
            const canManageSettings = canManageListSettings(activeList);
            listSettingsBox.classList.toggle("visible", canManageSettings);
            if (!canManageSettings) return;

            listSettingsInfo.textContent = "Besitzer: " + getUserName(activeList.ownerId) + " · Sichtbarkeit: " + (activeList.isShared ? "Gemeinschaftsliste" : "Privat");
            toggleSharedButton.textContent = activeList.isShared ? "Auf privat setzen" : "Als Gemeinschaft freigeben";
            ownerSelectRow.classList.toggle("visible", appData.isAdmin === true);
            activeListOwnerSelect.value = activeList.ownerId;
        }

        function renderAdminPanel() {
            adminPanel.classList.toggle("visible", appData.isAdmin === true);
            adminListTableBody.textContent = "";
        ...
        function renderAdminListTable() {
            ...
            let lists = appData.lists.slice();
            if (filter === "mine") lists = lists.filter(function(list) { return list.ownerId === appData.currentUser.id; });
            if (filter === "shared") lists = lists.filter(function(list) { return list.isShared; });
            if (filter === "private") lists = lists.filter(function(list) { return !list.isShared; });
            ...
            lists.forEach(function(list) {
                ...
                ownerCell.textContent = getUserName(list.ownerId);
                const scopeCell = document.createElement("td");
                scopeCell.textContent = list.isShared ? "Gemeinschaft" : "Privat";
                const itemCell = document.createElement("td");
                itemCell.textContent = String(list.items.length);
                ...
                appData.activeUsers.forEach(function(user) {
                    ...
                    if (user.id === list.ownerId) option.selected = true;
                    ownerSelect.appendChild(option);
                });

                const ownerButton = document.createElement("button");
                ownerButton.className = "small-btn";
                ownerButton.type = "button";
                ownerButton.textContent = "Besitzer ändern";
                ownerButton.addEventListener("click", function() { adminUpdateListOwner(list.id, ownerSelect.value); });

                const scopeButton = document.createElement("button");
                scopeButton.className = list.isShared ? "small-btn small-warning" : "small-btn small-success";
                scopeButton.type = "button";
                scopeButton.textContent = list.isShared ? "privat" : "freigeben";
                scopeButton.addEventListener("click", function() { adminToggleListVisibility(list); });

                actionsCell.appendChild(ownerSelect);
                actionsCell.appendChild(ownerButton);
        ...
        toggleSharedButton.addEventListener("click", updateActiveListVisibility);
        updateOwnerButton.addEventListener("click", updateActiveListOwner);
        adminListFilter.addEventListener("change", renderAdminPanel);
